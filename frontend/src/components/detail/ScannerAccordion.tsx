...
import { PILLAR_KEYS, PILLAR_LABELS, getGrade, hasDivergence, isStale } from "@/lib/scoring";
...
export function ScannerAccordion({ scanner, aggregatePillars }: Props) {
  ...
  const scannerStale = isStale(scanner.scored_at);
  ...
  return (
    <div className="border border-[#30363d] rounded-lg overflow-hidden mb-3 last:mb-0">
      ...
      >
        ...
        {scannerStale && (
          <span className="text-[11px] font-semibold bg-[#2d2a1a] text-[#ffa657] border border-[#ffa657] px-2 py-0.5 rounded-full whitespace-nowrap">
            ⚠ Stale (90d+)
          </span>
        )}
        <GradeBadge grade={grade} size="sm" variant="dark" />
        <span className="text-[13px] font-bold text-[#c9d1d9]">{Math.round(scanner.composite_score)}</span>
        {diverges && (
          <span className="text-[10px] font-semibold bg-[#2d2a1a] text-[#ffa657] border border-[#ffa657] px-[7px] py-[2px] rounded-full whitespace-nowrap">
            ↕ Diverges
          </span>
        )}
      ...
      {open && (
        <div className="px-4 pb-4 pt-3 bg-[#0d1117]">
          ...
          <div className={diverges ? "mt-2" : ""}>
            {PILLAR_KEYS.map((key) => {
              const score = scanner.pillars[key];
              const g = getGrade(score);
              return (
                <div key={key} className="grid grid-cols-[160px_28px_1fr_56px] items-center gap-2 py-1.5 border-b border-[#21262d] last:border-0 text-xs">
                  <span className="text-[#8b949e]">{PILLAR_LABELS[key]}</span>
                  <GradeBadge grade={g} size="sm" variant="dark" />
                  <ProgressBar score={score} grade={g} />
                  <span className="text-right font-semibold text-[#c9d1d9]">
                    {Math.round(score)}{" "}
                    <span className="text-[10px] font-normal text-[#8b949e]">/1000</span>
                  </span>
                </div>
              );
            })}
